  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/api/repeat-customers?interval=${interval}`)
      .then(response => {
        ...
        if (!Array.isArray(repeatCustomersData)) {
          console.error('Unexpected data format:', repeatCustomersData);
          setError('Data format error');
          return;
        }

        // Sort data based on the interval
        let sortedData = repeatCustomersData.slice(); // Clone array to avoid mutating original data
        ...
        let labels;
        const values = sortedData.map(item => item.repeatCustomers);
