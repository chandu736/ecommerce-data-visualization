  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/api/new-customers-over-time?interval=${interval}`)
      .then(response => {
        ...
        if (!Array.isArray(newCustomersData)) {
          console.error('Unexpected data format:', newCustomersData);
          setError('Data format error');
          return;
        }

        // Sort data based on the interval
        let sortedData = newCustomersData.slice(); // Clone array to avoid mutating original data
        ...
        let labels;
        const values = sortedData.map(item => item.newCustomers);
