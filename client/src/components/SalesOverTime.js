...
const SalesOverTime = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/api/sales-over-time?interval=${interval}`)
      .then(response => {
        const salesData = response.data || [];
        console.log('API response data:', salesData);

        if (!Array.isArray(salesData)) {
          console.error('Unexpected data format:', salesData);
          setError('Data format error');
          return;
        }

        // Sort data based on the interval
        let sortedData = salesData.slice(); // Clone array to avoid mutating original data
        ...
        let labels;
        const values = sortedData.map(item => item.totalSales);
